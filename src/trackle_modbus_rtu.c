...
ModbusError Trackle_Modbus_execute_command_rtu(TrackleModbusFunction function, uint16_t address, uint16_t start, uint16_t size, void *value)
{
    ...
    // write to serial
    if (MODBUS_WAIT_BETWEEN_PACKETS > 0)
    {
        uint32_t currentTick = xTaskGetTickCount();
        uint32_t targetTick = xLastWriteTime + MODBUS_WAIT_BETWEEN_PACKETS;
        int32_t ticksToWait = (int32_t)(targetTick - currentTick);

        ESP_LOGD(TAG_M, "MODBUS_WAIT_BETWEEN_PACKETS %lu %u %ld", currentTick, MODBUS_WAIT_BETWEEN_PACKETS, ticksToWait);

        if (ticksToWait > 0)
        {
            vTaskDelay(ticksToWait);
        }
        ESP_LOGD(TAG_M, "MODBUS_WAIT_BETWEEN_PACKETS end");
    }

    hal_uart_flush(); // very important!!!!
    ESP_LOGD(TAG_M, "starting rtu request");
    int len = hal_uart_write(modbusMasterGetRequest(&master), modbusMasterGetRequestLength(&master));
    ...
    len = hal_uart_read(modbus_uart_read_buffer, modbus_response_len(function, size), MODBUS_TIMEOUT);
    xLastWriteTime = xTaskGetTickCount();

    // parse response
    // ESP_LOG_BUFFER_HEX_LEVEL(TAG_M, modbus_uart_read_buffer, len, ESP_LOG_INFO);
    ...
    {
        ESP_LOGD(TAG_M, "Updating result value uint8_t buffer");
        for (int i = 0; i < size; i++)
        {
            int byte_index = i / 8;
            int bit_index = i % 8;

            if (modbus_read_buffer[i]) // Se il bit è 1, impostalo in value
            {
